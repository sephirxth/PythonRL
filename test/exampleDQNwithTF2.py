    ...
    def learn(self):
        ...
        if self.memory_counter > self.params['memory_size']:
            sample_index = np.random.choice(
                self.params['memory_size'], size=self.params['batch_size'])
        else:
            sample_index = np.random.choice(
                self.memory_counter, size=self.params['batch_size'])

        batch_memory = self.memory[sample_index, :]

        q_next = self.target_model.predict(
            batch_memory[:, -self.params['n_features']:])
        q_eval = self.eval_model.predict(
            batch_memory[:, :self.params['n_features']])
        ...
        q_target[batch_index, eval_act_index] = reward + \
            self.params['reward_decay'] * np.max(q_next, axis=1)
        ...
        self.cost = self.eval_model.train_on_batch(
            batch_memory[:, :self.params['n_features']], q_target)
...
def main():
    ...
    eval_model = Eval_Model(num_actions=action_dim)
    target_model = Target_Model(num_actions=action_dim)
    RL = DeepQNetwork(action_dim, state_dim, eval_model, target_model)
